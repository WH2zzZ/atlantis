...
import com.oowanghan.atlantis.util.base.Assert;
import com.oowanghan.atlantis.util.tool.finder.TextFinder;
...
public class SplitIter extends ComputeIter<String> implements Serializable {
	...
	public List<String> toList(boolean trim) {
		return toList((str) -> trim ? StringUtil.trim(str) : str);
	}
	...
	public <T> List<T> toList(Function<String, T> mapping) {
		...
		while (this.hasNext()) {
			...
			if (ignoreEmpty && StringUtil.isEmptyIfStr(apply)) {
